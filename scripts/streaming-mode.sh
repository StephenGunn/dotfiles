...
# Workspace 5 excluded - scrolling layout conflicts with pinned sidebar

workspace = 1, monitor:${MONITOR}, gapsout:15 ${SIDEBAR_WIDTH} 15 15
workspace = 2, monitor:${MONITOR}, gapsout:15 ${SIDEBAR_WIDTH} 15 15
workspace = 3, monitor:${MONITOR}, gapsout:15 ${SIDEBAR_WIDTH} 15 15
EOF
    else
        cat > "$STREAMING_CONF" <<EOF
# Streaming mode workspace gaps
...
apply_gaps() {
    ...
    if [[ "$enabled" == "on" ]]; then
        hyprctl --batch "\
            keyword workspace 1,monitor:${MONITOR} gapsout:15 ${SIDEBAR_WIDTH} 15 15; \
            keyword workspace 2,monitor:${MONITOR} gapsout:15 ${SIDEBAR_WIDTH} 15 15; \
            keyword workspace 3,monitor:${MONITOR} gapsout:15 ${SIDEBAR_WIDTH} 15 15"
    else
        hyprctl --batch "\
            keyword workspace 1,monitor:${MONITOR} gapsout:15 15 15 15; \
            keyword workspace 2,monitor:${MONITOR} gapsout:15 15 15 15; \
            keyword workspace 3,monitor:${MONITOR} gapsout:15 15 15 15"
    fi
}
